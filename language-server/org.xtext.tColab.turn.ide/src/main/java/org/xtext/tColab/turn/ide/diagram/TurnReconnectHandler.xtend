...
//			if (sourceElement instanceof IntentionalElement && targetElement instanceof IntentionalElement) {
//				val textEdits = newArrayList
//				val eventName = sourceElement.getContainerOfType(Actor)?.elems?.get() ?: 'undefined'
//				val transitionText = '''«'\n\t'»«eventName» => «(targetElement as Actor).name»'''
//				val oldRange = getOldRange(routable)
//				val newRange = getNewRange(sourceElement as Actor)
//				if (oldRange !== null) {
